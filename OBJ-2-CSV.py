...
# ==================== USER SETTINGS =======================
OUTPUT_HEADER = True     # Add header to CSV (Leave by default)
MIRROR_Z = True          # Mirrors Z (For original orientation)
# ==========================================================
...
def write_csv(csv_path, verts, uvs, faces):
    with open(csv_path, 'w', newline='') as f:
        ...
        if OUTPUT_HEADER:
            writer.writerow(["IDX", "VTX", "Position.x", "Position.y", "Position.z", "TexCoord.x", "TexCoord.y"])

        IDX = 0
        for VTX, face in enumerate(faces, start=1):
            for vert_idx in face:
                v = verts[vert_idx - 1]
                uv = uvs[vert_idx - 1] if vert_idx - 1 < len(uvs) else (0.0, 0.0)
                writer.writerow([VTX, IDX, v[0], v[1], v[2], uv[0], uv[1]])
                IDX += 1
